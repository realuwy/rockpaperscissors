  ...
  <body>
    ...
    <script>
      ...
      function initializeGame() {
        items = Array(10)
          .fill('🌑')
          .concat(Array(10).fill('📄'), Array(10).fill('✂'))
          .map((item, i) => {
            return {
              type: item,
              x: Math.random() * 400,
      ...
      function draw() {
        ...
        if (uniqueTypes.size === 1) {
          ...
          document.getElementById('announcement').style.color =
            predictedWinner === actualWinner ? 'green' : 'red';
        } else if (frameCount % 300 === 0) {
          // Increase the speed by 0.25 every 5 seconds (300 frames)
          for (let item of items) {
            item.dx += item.dx >= 0 ? 0.25 : -0.1;
            item.dy += item.dy >= 0 ? 0.25 : -0.1;
          }
        }
      }
